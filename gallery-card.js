        ...
        <div class="resource-menu-container">
          ${this.config.enable_date_search?L`
            <div class="card-header-actions">
              ...
              ${this._isDateFiltered?L`<span class="action-text btn-clear-date" @click="${this._clearDateFilter}">清除</span>`:L``}
            </div>
          `:L``}
            ...
            ${t.slice(0,this._itemsToShow).map(((e,t)=>L`
                    ...
                    ${e.isHass?L`
                          <hui-image
                            ...
                            .cameraView=${"live"}
                          ></hui-image>
                        `:this._isImageExtension(e.extension)?L`<img class="lzy_img" src="/local/community/gallery-card/placeholder.jpg" data-src="${e.url}"/>`:this.config.video_preload??1?L`<video preload="none" data-src="${e.url}#t=${void 0===this.config.preview_video_at?.1:this.config.preview_video_at}" @canplay="${()=>this._downloadNextMenuVideo()}" preload="metadata"></video>`:L`<div style="text-align: center"><div class="lzy_img"><ha-icon id="play" icon="mdi:movie-play-outline"></ha-icon></div></div>`}
